...
async function fetchAttachmentMetadata(cardId, attachmentId, key, token) {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`⚠️ Failed to fetch metadata for ${attachmentId} — status ${res.status}`);
      return null;
    }
...
async function uploadToAdpiler(cardId, env) {
  ...
  try {
    ...
    for (const att of attachments) {
      ...
      if (!att.isUpload) {
        ...
        if (!metadata || !metadata.id || !metadata.name) {
          console.warn(`⚠️ Invalid metadata for "${att.name}"`);
          continue;
        }
    ...
    if (!clientMatch) {
      console.error(`❌ Client "${matchKey}" not found in lookup sheet`);
      return;
    }
    ...
    // 4️⃣ Upload loop with baked-in guards
    for (const [index, item] of uploadQueue.entries()) {
      ...
      if (
        ...
        console.error(`❌ Skipping due to invalid name/url — name="${name}" url="${url}"`);
        continue;
      }
      ...
      try {
        ...
        if (!fileRes.ok) {
          console.error(`❌ Fetch failed for "${name}" — status: ${fileRes.status}`);
          continue;
        }
